...
describe("Validation of test file data", async () => {
  /*
  due to correction breach, le last event 'VotesTallied' is removed
  ...
  describe("the 'workflow' array corresponds to the 'workflowStatus' enum", function () {
    const testStatus = [
      ...
      { status: "VotingEndedAndTallied", expectedIndex: 4 },
    ];

    it("checks workflow has 5 statuses", function () {
      const statusCount = workflow.length;
      assert.equal(statusCount, 5, `workflow should have 5 statuses`);
    });
